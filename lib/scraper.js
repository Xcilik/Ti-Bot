...
async function ytMp4(url, options) {
	try {
		...
		const downloadVideo = new Promise((resolve, reject) => {
			...
			try {
				videoFormat = ytdl.chooseFormat(getUrl.formats, { quality: 'highestvideo', filter: format => format.container === 'mp4' && format.hasVideo && !format.hasAudio });
			} catch (e) {
				videoFormat = ytdl.chooseFormat(getUrl.formats, { quality: 'highestvideo', filter: 'videoonly' });
			}
