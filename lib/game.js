...
class Blackjack {
	...
	resolveRound() {
		...
		this.submitCard = [];
		this.skip = [];
		return `@${this.leader.split('@')[0]} memimpin ronde berikutnya!`;
	}
	...
	reuseSubmitCardsForDrinking() {
		...
		const drinkers = this.players.filter(p => !this.skip.includes(p.id));
		if (this.submitCard.length === 1 && this.isRoundComplete()) {
			const owner = this.submitCard[0].id;
			this.submitCard = [];
			this.leader = owner;
			for (const p of this.players) {
				if (p.id !== owner && !this.skip.includes(p.id)) {
					this.skip.push(p.id);
				}
			}
			return { msg: `Hanya @${owner.split('@')[0]} yang punya kartu — dia jadi pemimpin baru.`, continue: true };
		}
		cards.forEach((card, i) => {
			if (!drinkers.length) return;
			const p = this.players.find(x => x.id === drinkers[i % drinkers.length].id);
			if (!p) return;
			p.cards.push(card);
			if (!this.skip.includes(p.id)) this.skip.push(p.id);
		});
		this.submitCard = [];
		return { msg: 'Kartu dibagikan ke pemain yang harus minum.', continue: true };
	}
}
