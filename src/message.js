...
async function GroupParticipantsUpdate(naze, update, store) {
	try {
		...
		// If the bot itself is added to a group, run this check first!
		if (action === 'add') {
			const botNumber = jidNormalizedUser(naze.user.id);
			const isBotAdded = participants.some(p => jidNormalizedUser(p) === botNumber || jidNormalizedUser(p) === jidNormalizedUser(naze.user.lid));
			if (isBotAdded) {
				if (global.db && global.db.groups) {
					global.db.groups[id] = global.db.groups[id] || {
						url: '', text: {}, warn: {}, tagsw: {}, nsfw: false, mute: false, leave: false,
						setinfo: false, antilink: false, demote: false, antitoxic: false, promote: false,
						welcome: false, antivirtex: false, antitagsw: false, antidelete: false, antihidetag: false,
						waktusholat: false, remindersolat: false
					};
				}
				
				let metadata = store?.groupMetadata?.[id];
				if (!metadata) {
					metadata = await naze.groupMetadata(id).catch(e => {});
					if (metadata) {
				...
				if (metadata) {
					const isSewa = checkStatus(id, global.db.sewa);
					const ownerNumber = [...new Set([...global.owner, botNumber.split('@')[0], ...global.db.set[botNumber]?.owner || []])];
					const hasOwnerInGroup = metadata.participants?.some(p => {
					...
					if (!isSewa) {
						if (!hasOwnerInGroup) {
							...
							await naze.sendMessage(id, { text: welcomeSewa });
						} else {
							...
							await naze.sendMessage(id, { text: welcomeFree });
						}
					}
				}
			}
...
async function LoadDataBase(naze, m) {
	try {
		...
		if (setBot.template === 'documentMessage' || setBot.template === 3 || setBot.template === 1) {
			setBot.template = 2;
		}
...
async function Solving(naze, store) {
	...
	naze.sendFromOwner = async (jids, text, quoted, options = {}) => {
		for (const a of jids) {
			...
			const hasMessage = quoted && quoted.message && Object.keys(quoted.message).length > 0;
			await naze.sendMessage(jid, { text, ...options }, { ...(hasMessage ? { quoted } : {}), ephemeralExpiration: quoted?.expiration || quoted?.metadata?.ephemeralDuration || store?.messages[jid]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		}
	}
	...
	naze.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const interactiveButtons = buttons.map(a => {
			return {
				name: a.name,
				buttonParamsJson: typeof a.buttonParamsJson === 'string' ? a.buttonParamsJson : JSON.stringify(a.buttonParamsJson || '')
			};
		});
		...
		return sendInteractiveMessage(naze, jid, payload, options);
	}
	
	naze.sendButtonMsg = async (jid, content = {}, options = {}) => {
		if (content.viewOnceMessage) {
			return sendInteractiveMessage(naze, jid, content, options);
		}
		...
		const interactiveButtons = buttons.map((btn, i) => {
			...
			if (btn.nativeFlowInfo) {
				return {
					...
					buttonParamsJson: typeof btn.nativeFlowInfo.paramsJson === 'string' ? btn.nativeFlowInfo.paramsJson : JSON.stringify(btn.nativeFlowInfo.paramsJson)
				};
			}
		...
		if (media && typeof media === 'object' && Object.keys(media).length > 0) {
			const mediaType = Object.keys(media).find(key => ['image', 'video', 'document', 'location'].includes(key));
			if (mediaType) {
				const msg = await generateWAMessageContent(media, { upload: naze.waUploadToServer });
				const mediaMessage = msg[mediaType + 'Message'];
				if (mediaMessage) {
					mediaHeader = {
		...
		if (Object.keys(mediaHeader).length > 0) {
			interactiveMessage.header = mediaHeader;
